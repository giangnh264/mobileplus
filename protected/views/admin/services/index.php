...
$this->pageLabel = Yii::t("admin","Danh sách Services");
...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('services-model-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
if(Yii::app()->user->hasFlash('Services')){
    echo '<div class="flash-success">'.Yii::app()->user->getFlash('Services').'</div>';
}


$this->widget('zii.widgets.grid.CGridView', array(
	'id'=>'services-model-grid',
	...
	'columns'=>array(
		...
		array(
			'name' => 'Ảnh',
			'value' => 'CHtml::image($data->img_url)',
			'type' => 'raw'
		),
		'description',
		array(
            ...
            'header'=>CHtml::dropDownList('pageSize',$pageSize,array(10=>10,30=>30,50=>50,100=>100),array(
                                                                                  'onchange'=>"$.fn.yiiGridView.update('services-model-grid',{ data:{pageSize: $(this).val() }})",
                                                                                )),
