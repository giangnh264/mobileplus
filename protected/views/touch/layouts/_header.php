<?php echo '<?xml version="1.0" encoding="utf-8"?>'; ?>
<!DOCTYPE html PUBLIC "-//WAPFORUM//DTD XHTML Mobile 1.0//EN" "http://www.wapforum.org/DTD/xhtml-mobile10.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<link rel="icon" type="image/png" href="<?php echo Yii::app()->request->getHostInfo(). '/img/fav.png';?>">
<meta name="language" content="vi" />
<meta name="viewport" content="width=device-width; initial-scale=1.0; maximum-scale=1.0; user-scalable=0;"/>
<meta name="MobileOptimized" content="100" />

<?php include '_meta_tag.php'; ?>
<meta name="apple-itunes-app" content="app-id=1023317260">
<meta name="google-play-app" content="app-id=vn.com.vega.music.mobimusic">

	<meta name="msApplication-ID" content="App" />
	<meta name="msApplication-WinPhonePackageUrl" content="app-id=c4e37375-9c52-4778-9790-a6ff25efc57d" />
	<meta name="msApplication-PackageFamilyName" content="app-id=c4e37375-9c52-4778-9790-a6ff25efc57d" />
	<script type="text/javascript" src="<?php echo Yii::app()->createUrl("site/loadLang") ?>"></script>





<?php
$ie=false;
$iOS = false;
preg_match('/MSIE (.*?);/', $_SERVER['HTTP_USER_AGENT'], $matches);
if (count($matches)>1){
	$ie=true;
}else if(stripos($_SERVER['HTTP_USER_AGENT'],"iPod") || stripos($_SERVER['HTTP_USER_AGENT'],"iPhone")
	||stripos($_SERVER['HTTP_USER_AGENT'],"iPad")){
	$version = preg_replace("/(.*) OS ([0-9]*)_(.*)/","$2", $_SERVER['HTTP_USER_AGENT']);
	if($version<7) {
		$iOS = true;
	}
}
if(preg_match('/IEMobile/', $_SERVER['HTTP_USER_AGENT'])){
	$ie=true;
}
$deviceInfo = $this->device->getBasicInfo();

Yii::app()->clientScript->registerCssFile(Yii::app()->request->baseUrl.'/touch/css/style.css');
Yii::app()->clientScript->registerCssFile(Yii::app()->request->baseUrl.'/touch/css/main.css');
Yii::app()->clientScript->registerCssFile( Yii::app()->request->baseUrl.'/touch/css/jquery.smartbanner.css' );
Yii::app()->clientScript->registerCssFile( Yii::app()->request->baseUrl.'/touch/css/default.css' );
if($ie){
	//Yii::app()->clientScript->registerScriptFile(Yii::app()->request->baseUrl.'/touch/js/jquery.min.js');
}else{
	//Yii::app()->clientScript->registerScriptFile(Yii::app()->request->baseUrl.'/touch/js/zepto/zepto.min.js');
}
Yii::app()->clientScript->registerScriptFile(Yii::app()->request->baseUrl.'/touch/js/zepto/zepto.min.js');

Yii::app()->clientScript->registerScriptFile( Yii::app()->request->baseUrl.'/touch/js/zepto.smartbanner.js' );
Yii::app()->clientScript->registerScriptFile(Yii::app()->request->baseUrl.'/touch/js/common.js');
Yii::app()->clientScript->registerScriptFile(Yii::app()->request->baseUrl.'/touch/js/swipe-page.js');
Yii::app()->clientScript->registerScriptFile(Yii::app()->request->baseUrl.'/touch/js/base64.js');
Yii::app()->clientScript->registerScriptFile(Yii::app()->request->baseUrl.'/touch/js/base.js');
Yii::app()->clientScript->registerScriptFile(Yii::app()->request->baseUrl.'/touch/js/base.js');
if(Yii::app()->language=='km-kh'){
    Yii::app()->clientScript->registerCssFile(Yii::app()->request->baseUrl."/touch/css/kh.css?v=".time());
}
Yii::app()->clientScript->registerCssFile(Yii::app()->theme->baseUrl."/touch/css/style.css" );

Yii::app()->clientScript->registerScript("main","
		var deviceOs = '". ($this->deviceOs?(strtoupper($this->deviceOs)):'')."';
		var userPhone = ". ($this->userPhone?$this->userPhone:'false').";
		var userSubs = ". (!empty($this->userSub)?'true':'false').";
		var msgDetect = '".Yii::app()->params['MSG_NO_DETECT']."';
		var msgErrorCharg = '".Yii::app()->params['MSG_ERR_CHARG']."';
		var urlCharg = '".Yii::app()->createUrl('default/charge')."';
		var urlDownload = '".Yii::app()->createUrl('default/download')."';
		var urlLogPlayWifi = '".Yii::app()->createUrl('ajax/logPlayWifi')."';
		var urlLimitPlayWifi = '".Yii::app()->createUrl('ajax/limitPlayWifi')."';
		var urlCheckPlayWifi = '".Yii::app()->createUrl('ajax/checkPlayWifi')."';
		var urlCheckPlayNoSub = '".Yii::app()->createUrl('ajax/checkPlayNoSub')."';
		var urlLimit = '".Yii::app()->createUrl('ajax/limit')."';
		var urlLimitCtkm = '".Yii::app()->createUrl('ajax/limitCtkm')."';

		var debug_mode = '".YII_DEBUG."';
		var page_id = '".Yii::app()->controller->id."-".$this->action->id."';
		var os = '".$this->deviceOs."';
		var os_version = '".$deviceInfo['device_os_version']."';
        var rootPath = '".Yii::app()->params['base_url']."';
",CClientScript::POS_HEAD);

if($iOS) {
	Yii::app()->clientScript->registerScript('ios-fixed','
	var focusing = false;
	jQuery(document).ready(function(){
		$("input").on("focus", function() {
				$("header").css({
					"position": "absolute",
					"top": jQuery(document).scrollTop()
				});
				focusing = true;
				setTimeout(function () {
					console.log("done");
					window.scrollTo(document.body.scrollLeft, document.body.scrollTop);
				}, 0);
			});
	});

	jQuery(window).scroll(function() {
		if ( focusing ) $("header").css({ "top": jQuery(document).scrollTop() });
	});
	',CClientScript::POS_READY);
}

?>

	<?php //include '_ga.php'; ?>
</head>
<body>
<script type="text/javascript">
	var android = false;
	var winphone = false;



	if( navigator.userAgent.match(/Android/i)){
		android = true;
		if(navigator.userAgent.match(/Windows Phone/i)){
			winphone = true;
			android = false;
		}
	}

	if(android){
		$.smartbanner({
			title: 'Amusic - Nghe Nhạc Free 3G Mobifone',
			author: 'VEGA CORPORATION',
			price: 'FREE',
			button: 'Cài đặt',
			icon: '<?php echo Yii::app()->request->baseUrl.'/touch/images/amusic.png';?>',
			force: android ? 'android' : (winphone ? 'windows':'ios'),
			daysHidden: 1,
			daysReminder: 30
		});
	}
</script>
<a id="top"></a>
<?php echo $content;?>
<div id="popup_mask" class="none" style="height: 1500px;"></div>
<div id="vg_popup" class="none"></div>
<div id="overlay-bg"></div>

<?php //include '_ads_marketing.php'; ?>
<?php /*
<script type="text/javascript">
	$(function() { $.smartbanner() } )
</script>
*/?>
</body>
</html>