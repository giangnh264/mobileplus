...
class WPN{
    ...
    public function push($uri, $xml_data, $type = WPNTypesEnum::Raw, $tileTag = ''){
        try {
            if ($this->access_token == '') {
            ...
            $headers = array('Content-Type: ' . $contentType, "Content-Length: " . strlen($xml_data), "X-WNS-Type: $type", "Authorization: Bearer $this->access_token");
            if ($tileTag != '') {
        ...
        {
            $code = '-1';
            return new WPNResponse('Exception:'.$e->getMessage(), $code, true);
        }
    }
}
class WPNTypesEnum{
