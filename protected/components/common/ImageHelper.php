...
class ImageHelper
{
	...
	public function resizeImage($source,$width,$height,$type='ratio')
	{
		...
		if(!file_exists($assetPath) && !mkdir($assetPath,0755)) return $source;
		//$folders = explode('/',$source);
		$folders = explode('/',$rzname);
		$tmppath = $assetPath.DS;
		for($i=0;$i < count($folders)-1; $i++){
			if(!file_exists($tmppath.$folders[$i]) && !mkdir($tmppath.$folders[$i],0755)) return $source;
			$tmppath = $tmppath.$folders[$i].'/';
		}
		if(!file_exists($assetPath."/tmp")) mkdir($assetPath."/tmp",0755);
