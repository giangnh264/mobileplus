...
class XUploadAction extends CAction
{
	...
	public function run()
	{
		...
		if(count($this->alowType) > 0 && !in_array($model->mime_type, $this->alowType)){
			echo json_encode(array("error"=>"1","msg"=>Yii::t('admin','Không hỗ trợ định dạng {type}',array('{type}'=>$model->mime_type)) )); exit;
		}
		...
		$mime = $model->file->getTempName();
		$fileMime = mime_content_type($mime);
		if(!in_array($fileMime, $this->alowType)){
			echo json_encode(array("error"=>"1","msg"=>Yii::t('admin','File ảnh không hợp lệ') )); exit;
		}
		if ($model->validate()) {
